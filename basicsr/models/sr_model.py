...
class SRModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.cri_align or self.cri_teacher:
            self.output, self.attn_out, ref = self.net_g(self.lq)
        else:
        ...
        # teacher loss
        if self.cri_teacher:
            l_teacher = self.cri_teacher(self.attn_out, self.gt)
            l_total += l_teacher
            loss_dict['l_teacher'] = l_teacher
        # alignment loss
        if self.cri_align:
            l_total += self.cri_align_loss_weight * ref
            loss_dict['l_align'] = ref
        # perceptual loss
